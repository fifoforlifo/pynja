...
def _emit_vsproj_2008(projectMan, projName, variants):
...
''')
    ...
    for i, variantName in enumerate(variantNames):
        ...
        strings.append(_def_config(variantName, ninjaDir, proj.outputPath, proj))
    strings.append(_def_config("all", ninjaDir, projName, firstProj))
...
def _emit_vssln_2008(vsProjList, vsSlnPath, variantNames):
    strings = []
...
def emit_vs_projects(projectMan):
    ...
    if projectMan.emitVS2008Projects:
        ...
        for projName in sorted(projectMan._projects.keys()):
            ...
            vsProjInfo = _emit_vsproj_2008(projectMan, projName, variants)
            vsProjList.append(vsProjInfo)
        _emit_vssln_2008(vsProjList, os.path.join(os.path.dirname(projectMan.ninjaPath), "vs2008.sln"), variantNames)
